...
public class PSEntityListener implements Listener {
    ...
    public void onEntityDamage(EntityDamageEvent event) {
        ...
        if (event.getEntity() instanceof WaterMob) {
            Field field = plugin.getForceFieldManager().getEnabledSourceField(event.getEntity().getLocation(), FieldFlag.PROTECT_WATERMOBS);
            ...
            if (field != null) {
                if (event instanceof EntityDamageByEntityEvent) {
                    ...
                    if (player != null) {
                        if(FieldFlag.PROTECT_WATERMOBS.applies(field, player)){
                            event.setCancelled(true);
                            return;
                        }
                    } else {
						if(field.hasFlag(FieldFlag.PROTECT_WATERMOBS)){
						    event.setCancelled(true);
						    return;
                        }
                    }
    ...
    public void onEntityBlockForm(EntityBlockFormEvent event) {
        ...
        // -------------------------------------------------------------------------------------- prevent frost walking block place
        if (event.getEntity() instanceof Player) {
            BlockState blockState = event.getNewState();

            if (blockState.getType().equals(Material.FROSTED_ICE)) {
